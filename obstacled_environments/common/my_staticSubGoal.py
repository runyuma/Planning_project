...
class StaticSubGoal(SubGoal):
    ...
    def update_position(self, new_position):
        self._config.desired_position = new_position
    # yongxi
    # 本来在static goal中不需要update位置，这里为了方便在static基础上改动
    def update_bullet_position(self, pybullet, **kwargs):
        # self._config.desired_position = new_position
        # # self._content_dict.desired_position = new_position
        pos = self.position().tolist()
        # if self.dimension() == 2 and self.indices() == [0, 1]:
        #     pos += [0.0]
        ori = [0, 0, 0, 1]
        # print(self._bullet_id)
    ...
    def render_gym(self, viewer, rendering, **kwargs):
        ...
        if self.dimension() == 1:
            ...
            x = self.position()
            # print(x)
            goal = viewer.draw_circle(self.epsilon(), color=[1.0, 0.0, 0.0])
...
class GlobalStaticSubGoal(SubGoal):
    def __init__(self, **kwargs):
        schema = OmegaConf.structured(StaticSubGoalConfig)
        super().__init__(schema, **kwargs)
        self.check_completeness()
